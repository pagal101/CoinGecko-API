...
class CoinGecko {
  ...
  _buildRequestOptions(path, params) {
    ...
    if (Utils.isObject(params)) {
      if (this._apiKey !== undefined) params['x_cg_pro_api_key'] = this._apiKey
      params = querystring.stringify(params);
    }
    else {
      params = (this._apiKey !== undefined) ? querystring.stringify({ 'x_cg_pro_api_key': this._apiKey }) : undefined;
    }
    ...
    return {
      path,
      method: 'GET',
      hostname:`${(this._apiKey !== undefined) ? 'pro-' : ''}${Constants.HOST}`,
      port: 443,
